      ...
      if (
        ...
        if (
          ...
          if (test === 'status-replacement') {
            ...
            const writerStage = writerRecord?.[1];
            ...
            if (writerCode && validScalars) {
              const stageCategory = writerStage ? `.${writerStage}` : '';
              return `${base}.assertion.writer-error${stageCategory}.${writerCategories[writerCode]}`;
            }
            const categories = [
