...
- Modify: `src/specification-guards.test.ts:231` (the capability exact-equality list)
...
**Constraints from the keyring implementation** — from Task 2's code review, now that Task 2 has
landed. The code below already respects all three; keep them true of any change to it.

- The keyring mutation lock is not reentrant: a nested acquisition spins for 5 s and then fails with
  `Contended`. `identity_status` and `identity_sign_in` call `read_session` and then `write_session`
  **sequentially** and never hold either across the other.
- `write_session` returning `Ok(false)` means another writer won and nothing was written. Never loop
  unbounded on it: retry at most once — re-read, rebuild `next`, write again — and then surface the
  `Sign-in changed in another window. Retry.` error.
- `StoredSession` implements `Drop`, so its fields cannot be moved out; clone them.
...
Also update `src/specification-guards.test.ts`: in the exact-equality expectation (~line 231, right after the `...covenRuntimeCommands.map(...)` spread) insert
...
`keeps the default Tauri capability least-privileged for native connection commands` asserts `capability.permissions` with `toEqual`, so it fails on the four additions above until that list matches. Run `corepack pnpm exec vitest run src/specification-guards.test.ts` and add the file to this task's `git add`.
...
```bash
git add src-tauri/src/identity.rs src-tauri/src/keyring.rs src-tauri/src/lib.rs src-tauri/build.rs src-tauri/capabilities/default.json src-tauri/gen/schemas/desktop-schema.json src/specification-guards.test.ts
...
Also update `src/specification-guards.test.ts`: in the same exact-equality expectation Task 9 edited, insert right after `'allow-identity-sign-out',`
...
Run `corepack pnpm exec vitest run src/specification-guards.test.ts` and add the file to this task's `git add`.
...
```bash
git add src-tauri/src/setup.rs src-tauri/src/coven_runtime.rs src-tauri/src/lib.rs src-tauri/build.rs src-tauri/capabilities/default.json src-tauri/gen/schemas/desktop-schema.json src/specification-guards.test.ts
...
export type SignInEvent = {
  type: 'waiting' | 'received' | 'exchanged' | 'done' | 'error';
  text?: string;
};
...
describe('setup client', () => {
  ...
  it('exposes the exact display commands and maps cancel', async () => {
    ...
    const setup = createSetup({ available: () => true, invoke });
    expect(setup.displayCommand('install_cli')).toBe(
      'npm install -g --prefix ~/.local @opencoven/cli',
    );
...
const UNAVAILABLE =
  'Local Coven requires the desktop app. Install Coven CLI and open OpenCoven Chat.';
export const ROW_KEYS: readonly SetupRowKey[] = [
  'platform',
  'node',
  'npm',
  'coven',
...
Append both tests at the end of `describe('production Familiars layout', …)` in
`src/coven/chat-layout.test.tsx`, immediately before `describe('compact viewports', …)`. That
file has no shared props constant: it builds props with `function layoutProps(): ChatLayoutProps`,
so spread `{...layoutProps()}`. `render`, `fireEvent`, `screen` and `vi` are already imported
there.
...
```tsx
  ...
  it('renders the account row even when archive filtering is unavailable', () => {
    render(<ChatLayout {...layoutProps()} account={{ email: 'v@x', onSignOut: () => {} }} />);
    fireEvent.click(screen.getByText('User settings'));
...
The `| undefined` is required: `tsconfig.json` sets `exactOptionalPropertyTypes`, and Task 18 and
`chat-app.tsx` both forward an `account` that may be `undefined`.
...
and where `<ChatLayout` is rendered (line 471) add `account={account}`.
...
import { describe, expect, it, vi } from 'vitest';
import type { Identity, SignInEvent } from '../lib/identity';
import type { Setup, SetupExit, SetupLine, SetupStep } from '../lib/setup';
...
describe('Onboarding', () => {
  ...
  it('offers sign-in when signed out and streams progress', async () => {
    ...
    const { identity, setup } = fakes({
      identity: {
        signIn: vi.fn(
          (_runId: string, onEvent?: (event: SignInEvent) => void) =>
    ...
    (identity.status as ReturnType<typeof vi.fn>).mockResolvedValue({
      state: 'entitled',
      email: 'v@x',
    });
  ...
  it('explains expiry and revocation on the sign-in screen', async () => {
    for (const [state, text] of [
      ...
      const { identity, setup } = fakes({
        identity: { status: vi.fn().mockResolvedValue({ state }) },
      });
      const { unmount } = render(
        <Onboarding identity={identity} setup={setup} available={() => true} />,
      );
  ...
  it('renders prerequisites with a Run button only for coven and engine', async () => {
    const report = {
      ...green,
      familiar: { ok: false, detail: 'No familiar with a workspace was found.' },
      coven: { ok: false, detail: 'Coven CLI was not found.' },
    };
    let finish: (exit: SetupExit) => void = () => {};
    const { identity, setup } = fakes({
      identity: { status: vi.fn().mockResolvedValue({ state: 'entitled', email: 'v@x' }) },
      setup: {
        check: vi.fn().mockResolvedValueOnce(report).mockResolvedValue(green),
        run: vi.fn(
          (_runId: string, _step: SetupStep, onLine?: (line: SetupLine) => void) =>
            new Promise<SetupExit>((resolve) => {
              onLine?.({ type: 'stdout', text: 'added 12 packages' });
              finish = resolve;
            }),
        ),
      },
    ...
    render(<Onboarding identity={identity} setup={setup} available={() => true} />);
    expect(await screen.findByText('No familiar with a workspace was found.')).toBeInTheDocument();
    ...
    expect(await screen.findByText('added 12 packages')).toBeInTheDocument();
    await act(async () => {
      finish({ type: 'exit', code: 0, cancelled: false });
    });
    expect(await screen.findByTestId('chat-app')).toBeInTheDocument();
  });

  it('offers no Run button and shows the Node.js address when Node is missing', async () => {
    const report = {
      ...green,
    ...
    const { identity, setup } = fakes({
      identity: { status: vi.fn().mockResolvedValue({ state: 'entitled', email: 'v@x' }) },
      setup: { check: vi.fn().mockResolvedValue(report) },
    });
    render(<Onboarding identity={identity} setup={setup} available={() => true} />);
    expect(await screen.findByText('Node.js was not found on this machine.')).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Run' })).toBeNull();
    expect(screen.getByText('nodejs.org/en/download')).toBeInTheDocument();
  });

  it('shows exit code and last lines on a failed install', async () => {
    const report = {
      ...green,
      engine: { ok: false, detail: 'The Coven engine is not installed.' },
    };
    const { identity, setup } = fakes({
      identity: { status: vi.fn().mockResolvedValue({ state: 'entitled', email: 'v@x' }) },
      setup: {
        check: vi.fn().mockResolvedValue(report),
        run: vi.fn().mockResolvedValue({ type: 'exit', code: 2, cancelled: false }),
      },
    });
    render(<Onboarding identity={identity} setup={setup} available={() => true} />);
    ...
    const { identity, setup } = fakes({
      ...
      setup: {
        check: vi.fn().mockResolvedValue({
          ...green,
          platform: { ok: false, detail: "Chat isn't available on Windows yet." },
        }),
      },
    });
    render(<Onboarding identity={identity} setup={setup} available={() => true} />);
  ...
  it('passes a working sign-out to the chat and returns to sign-in', async () => {
    const { identity, setup } = fakes({
      identity: {
        status: vi
          .fn()
          .mockResolvedValueOnce({ state: 'entitled', email: 'v@x' })
          .mockResolvedValue({ state: 'signed_out' }),
      },
    });
    render(<Onboarding identity={identity} setup={setup} available={() => true} />);
...
`src/onboarding/onboarding.css` (tokens only; no hex literals). `--border-hairline`,
`--color-success` and `--color-danger` are declared on `.fr-shell` in
`src/design/familiars-shell.css`, so they do not reach `.ob-shell`; each use below carries the
same value as its fallback. `--surface-base`, `--surface-raised`, `--text-muted` and
`--text-secondary` are `:root` tokens in `src/styles.css` and need none:
...
.ob-row[data-ok="true"] {
  border-color: var(--color-success, var(--text-secondary));
}
.ob-row-head {
  display: flex;
  align-items: center;
...
.ob-error {
  color: var(--color-danger, oklch(0.66 0.11 24));
  font-size: 13px;
}
```
...
const STEP_FOR: Partial<Record<SetupRowKey, SetupStep>> = {
  coven: 'install_cli',
...
type RunState = {
  runId: string;
  lines: string[];
  exit?: { code: number; cancelled: boolean };
  error?: string;
};
...
export function Onboarding({
  ...
  const [identity] = useState(() => identityOverride ?? createIdentity());
  const [setup] = useState(() => setupOverride ?? createSetup());
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      setPhase(
        next({ kind: 'error', text: error instanceof Error ? error.message : String(error) }),
      );
    }
    ...
    try {
      const exit = await setup.run(id, step, (line: SetupLine) =>
        setRuns((r) => {
          ...
          return {
            ...r,
            [key]: { ...current, lines: [...current.lines, line.text].slice(-LAST_LINES) },
          };
        }),
      );
  ...
  return (
    <div className="ob-shell">
      <section className="ob-card" aria-label="Onboarding">
                  ...
                  <div key={key} className="ob-row" data-ok={row.ok}>
                    ...
                    {!row.ok && step ? (
                      <>
                        ...
                        {state?.exit?.cancelled ? (
                          <span className="ob-muted">Cancelled.</span>
                        ) : null}
                    ...
                    {!row.ok && (key === 'node' || key === 'npm') ? (
                      <span className="ob-muted">
                        Install Node.js from <code>nodejs.org/en/download</code>, then check again.
                      </span>
                    ) : null}
                  </div>
                );
...
`main.tsx` renders inside `<StrictMode>`, so in development the mount effect runs twice and
`identity_status` is called twice at startup; this is expected and idempotent.

The Node.js row deliberately prints the address as text rather than linking it. The Tauri webview
has no opener permission, and `src/specification-guards.test.ts` (`keeps runtime source free from ad
hoc Cave networking primitives`) rejects any `http://` or `https://` literal anywhere under `src`,
so an anchor would both fail the guard and do nothing when clicked. Do not add a guard exception.
Before committing, confirm `grep -n 'http' src/onboarding/onboarding.tsx` prints nothing.
...
Run `corepack pnpm exec biome check --write src/onboarding` before `pnpm lint`; accept its
formatting. The code above is already written at Biome's 100-column shape, but JSX reflow is
easier to let the formatter settle than to hand-match.
...
- Modify: `src/single-entrypoint.test.ts:7-9`
- Create: `e2e/tauri-mock.ts`
- Modify: `e2e/app.spec.ts`, `e2e/app.smoke.spec.ts`, `e2e/responsive-shell.spec.ts`,
  `e2e/local-storage.spec.ts`, `e2e/app.tauri-mock.spec.ts`, `e2e/chat-lifecycle.spec.ts`

- [ ] **Step 1: Update the guard first (it must fail, then pass)**

Replace lines 7–9 of `src/single-entrypoint.test.ts` with:
...
- [ ] **Step 3: Update the Playwright suite for the gate**

`main.tsx` now renders `Onboarding`, which calls `identity_status` — and then `setup_check` — before
any chat surface mounts. Every Playwright spec has to answer those two commands. The four specs that
ran with no native mock at all now render the "requires the desktop app" card instead of `.fr-shell`,
and the two that install their own fixture reach the `default:` arm and throw. Fix the suite here or
`pnpm test:e2e` fails.
...
/**
 * Installs `window.__TAURI_INTERNALS__` before the app boots.
 *
 * The onboarding gate runs ahead of chat, so every spec needs `identity_status` and `setup_check`
 * to answer before anything renders. Those two defaults go in first; a handler passed here for the
 * same command replaces the default. Unknown commands still throw.
 *
 * Handlers are serialised with `Function.prototype.toString()` and re-parsed inside the page,
 * exactly like the function form of `page.addInitScript`: each one must be self-contained, with no
 * closure variables. Shared page-side state belongs on `window.__tauriMock`.
 */
export async function installTauriMock(page: Page, handlers: Record<string, Handler>) {
  const entries = Object.entries(handlers)
    .map(([command, handler]) => `${JSON.stringify(command)}: ${handler.toString()}`)
    .join(',\n');
  await page.addInitScript(`(${mock.toString()})({\n${entries}\n});`);
}

/** Runs inside the page; must not reference anything outside its own body. */
function mock(handlers: Record<string, Handler>) {
  const ok = { ok: true };
  ...
  const table: Record<string, Handler> = Object.assign({}, defaults, handlers);
  const callbacks = new Map<number, (data: unknown) => void>();
  const calls: string[] = [];
  let callbackId = 0;
  const scope = window as typeof window & { __tauriMock?: unknown };
  scope.__tauriMock = { callbacks, calls };
  Object.defineProperty(window, '__TAURI_INTERNALS__', {
    value: {
      ...
      async invoke(command: string, args: Record<string, unknown> = {}) {
        ...
        if (!handler) throw new Error(`Unexpected native command: ${command}`);
        return handler(args);
      },
    },
  });
...
`Object.assign` rather than object spread, and no helper functions outside `mock`, because `mock`
is shipped to the page as source text: anything the bundler would hoist out of it would be
undefined there. `e2e/tauri-mock.ts` is not a `*.spec.ts`, so Playwright will not collect it as a
test file, but `pnpm lint` does cover `e2e`.

**The four browser-only specs.** Each gets the import, a shared handler map, and one
`installTauriMock` call before every `page.goto`. The mock makes the app look like the desktop
shell, so the browser-runtime notice no longer renders: every
`await expect(page.getByText(/desktop app/i).first()).toBeVisible();` becomes an assertion on the
mocked runtime line instead, `await expect(page.getByText('Coven CLI · local sessions')).toBeVisible();`
(the status `chat-app.tsx` builds from `Coven ${health.version ?? 'CLI'} · local sessions`).

`e2e/app.spec.ts` in full — note the renamed title, since the test no longer demonstrates a browser
runtime limit, and the added negative assertion that pins the notice as gone:

```ts
import { expect, test } from '@playwright/test';
import { installTauriMock } from './tauri-mock';

test('opens the Familiars layout without Cave and reports the local runtime', async ({ page }) => {
  await installTauriMock(page, {
    coven_runtime_status: () => ({ available: true, transport: 'cli' }),
  ...
  await expect(page.getByText('Coven CLI · local sessions')).toBeVisible();
  await expect(page.getByText(/desktop app/i)).toHaveCount(0);
...
`e2e/app.smoke.spec.ts`: add after line 1
...
then `await installTauriMock(page, localRuntime);` immediately above each of the three
`page.goto` calls (lines 5, 19 and 34 of the current file), and replace the two
`getByText(/desktop app/i).first()` assertions (lines 9 and 21) with the
`'Coven CLI · local sessions'` assertion above.

`e2e/responsive-shell.spec.ts`: add the same import and `localRuntime` constant after line 1, then
`await installTauriMock(page, localRuntime);` immediately above each of the three `page.goto('/')`
calls (lines 36, 52 and 86). It asserts nothing about the desktop notice, so nothing else changes.

`e2e/local-storage.spec.ts`: add the same import and constant after line 1 and
`await installTauriMock(page, localRuntime);` above the first `page.goto('/')` (line 9 — one call
covers the later navigations and the reload, because init scripts run on every navigation), then
replace both `getByText(/desktop app/i).first()` assertions (lines 61 and 63) with the
`'Coven CLI · local sessions'` assertion.

**The two specs that already install a fixture.** `installTauriMock` serialises its handlers, so it
cannot carry theirs: `e2e/app.tauri-mock.spec.ts` closes over the per-test `ready`/`reply`/
`population` parameters, a live `callbacks` map and `rejectRun`, and `e2e/chat-lifecycle.spec.ts`
closes over its `session` record. Leave both fixtures alone and answer the two gate commands inside
their own `switch`.

In `e2e/app.tauri-mock.spec.ts`, replace lines 178–179
...
That file also records every command in `window.__covenFixture.calls`, and two assertions now see
the two gate commands ahead of the runtime ones. Replace line 293
...
```ts
  expect(await page.evaluate(() => window.__covenFixture.calls)).toEqual([
    'identity_status',
    'setup_check',
...
In `e2e/chat-lifecycle.spec.ts`, replace lines 49–50
...
```ts
            case 'identity_status':
              return { state: 'entitled', email: 'e2e@example.com', checkedAt: 1, graceUntil: 2 };
            case 'setup_check': {
              const green = { ok: true };
              return {
                platform: green,
...
Run: `corepack pnpm exec biome check --write e2e && corepack pnpm test:e2e`
Expected: every spec passes. If a spec fails on a layout assertion rather than on the gate, the
mocked runtime is reporting `available: true` where that spec expected the empty browser state —
give that spec's `coven_runtime_status` handler the shape its assertions need rather than loosening
the assertion.
...
```bash
git add src/main.tsx src/single-entrypoint.test.ts e2e
...
test('capabilities grant exactly the seven onboarding permissions', () => {
  ...
  const onboarding = granted.filter(
    (p) => p.startsWith('allow-identity-') || p.startsWith('allow-setup-'),
  );
  expect(onboarding.sort()).toEqual([...expected].sort());
});
```

The exact-equality list in `src/specification-guards.test.ts` was already updated by Tasks 9 and
12; this test adds the set-based check that no other `allow-identity-*` or `allow-setup-*`
permission creeps in later.
