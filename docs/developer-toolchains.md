...
- CI requires the GitHub Actions repository variables
  `OPENCOVEN_SDK_REVIEWED_REVISION` and
  `OPENCOVEN_CAVE_REVIEWED_REVISION`, each set to a reviewed immutable
  40-character commit SHA before external checkouts run.
