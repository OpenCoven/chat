...
- This production-consumer preparation uses the replacement private SDK
  ...
  historical. Native production sources and the Phase 1 producer lock are
  unchanged; a separately reviewed producer binding and validator freeze are
  still required. Local packed-consumer verification is not accepted
  conformance evidence or publication authorization.
